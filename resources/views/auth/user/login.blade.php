...
@section('title', 'Subscriber Account Login - Blue Mail')
@section('description', 'The subscriber account login page of Blue Mail.')
@section('keywords', 'Subscriber Account Login Blue Mail')
@section('content')
    <div class="grid grid-cols-1 p-3">
        <div
            ...
            <h1 class="px-2 text-2xl text-center">{{ __('site.subscribe_login_form') }}</h1>
            <div class="px-2 py-4">
                <form action="{{ url('/subscriber/login') }}" method="POST">
                    @csrf
